...
sap.ui.define([
	...
	return UI5Object.extend("ewm.zewmzpickingapp.controller.ErrorHandler", {
		...
		_displayTextMessage: function (sMessage, sType) {
			...
			var isJSONFormatData = true;
            var isJSONData = this.isJSON(sMessage);
            if(isJSONData)
            {
                sMessage = JSON.parse(sMessage);
                var innerError = sMessage.error.innererror;

                if (innerError.hasOwnProperty("errordetails") && Array.isArray(innerError.errordetails)) {
					sMessage = innerError.errordetails;
                }
                else
                {
                    sMessage = sMessage.error.message.value;
					isJSONFormatData = false;
                }
			...
			else
			{
				...
				if(sMessage.length > 1)
				{
					...
					for(var a = 0; a < sMessage.length; a++)
					{
						var errorMess = JSON.parse(sMessage[a]);
						//sMessage = JSON.parse(sMessage[a]);
						arrMessag.push(errorMess.error.innererror.errordetails);
					}
					//data like this [0][0]
					//Nested data
					for(var i = 0; i < arrMessag.length; i++)
					{
						for(var a = 0; a < arrMessag[i].length; a++)
						{
							errorArrMess.push(arrMessag[i][a]);
						}
					}

					sMessage = errorArrMess;
				}
			}
			...
			// if there is not already an identical message in it:
			this._addMessageToMessages(oMessage, sMessage, isJSONFormatData);
		 ...
		 */
		_addMessageToMessages: function (oMessage, sMessage, isJSONFormatData) {
			var bFound = false;
			...
			if (!bFound) {
				...
				if(isJSONFormatData)
				{
					...
					// {
						...
						{
							var severityStatus = this._massageSeverityStatus(sMessage[i]);
						   
							var oMessage = {
								message: sMessage[i].message,
								type: severityStatus
							};
							multipleMessage.push(oMessage);
	
						}
						this._aMessages = multipleMessage;
					//}
				}
                else
                {
		...
		_setPunctuationMark: function (sMessage) {
			if (sMessage.length > 0) {
				...
				if (sLastLetter === "." | sLastLetter === "!" | sLastLetter === "?" | sLastLetter === ";" | sLastLetter ===
					...
					// return sMessage + ".";  // this one need to comment because in the json file, is not allow to have . special character 
                    return sMessage;
				}
			}
			return sMessage;
        ...
        /**
		 * The isJSON method returns the JSON data whether is true or false
		 * @private
		 * @method module:controller/ErrorHandler#isJSON
		 * @returns boolean for json data consist of array json data.
		 */
        isJSON: function(sMessage) {
            try {
              JSON.parse(sMessage);
              return true; // The input is in valid JSON format.
            } catch (error) {
              return false; // The input is not in valid JSON format.
            }
        },

        _massageSeverityStatus: function(sMessage)
        {
            if(sMessage.severity.toUpperCase() == "ERROR")
            {
                sMessage.severity = "Error";
            }
            else if (sMessage.severity.toUpperCase() == "WARNING")
            {
                sMessage.severity = "Warning";
            }
            else if (sMessage.severity.toUpperCase() == "INFORMATION")
            {
                sMessage.severity = "Information";
            }
            else if (sMessage.severity.toUpperCase() == "SUCCESS")
            {
                sMessage.severity = "Success";
            }

            return sMessage.severity;
        }
	});
});